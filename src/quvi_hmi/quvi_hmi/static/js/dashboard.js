...
// ─── 프린터(Mainsail 임베드) ───
// 프린터 탭은 Mainsail을 통짜 iframe 임베드. src는 moonraker_url 호스트에서 조립(포트 없이 nginx 80).
(async function initMainsailFrame() {
    const frame = document.getElementById('mainsailFrame');
    if (!frame) return;
    try {
        const res = await fetch('/api/devices');
