...
if [ -t 0 ]; then
    docker exec -it "${TARGET_CONTAINER}" bash -c "source /workspace/install/setup.bash && ros2 launch quvi_bringup full_system.launch.py"
else
    docker exec -i "${TARGET_CONTAINER}" bash -c "source /workspace/install/setup.bash && ros2 launch quvi_bringup full_system.launch.py"
fi
