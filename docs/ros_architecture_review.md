...
> ## 본문은 2026-06-16 시점 평가입니다 — 아래 지적사항 대부분은 이후 해소되었습니다
>
...
QUVI 프로젝트는 ROS 2 Jazzy 기반으로 `quvi_msgs`, `quvi_inspect`, `quvi_robot_control`, `quvi_hmi`, `quvi_bringup`가 역할별로 분리되어 있다. 전체 구조는 시연용 통합 프로토타입으로는 방향이 잡혀 있지만, 실제 장비를 안정적으로 자동 운전하기에는 상태 피드백, 비상정지 전파, 하드웨어 완료 신호, 모델/파라미터 정합성에서 보완이 필요하다.
...
`quvi_inspect`는 `quvi_robot_control.utils`를 직접 import한다. 하지만 `package.xml`에는 `quvi_robot_control` 의존성이 선언되어 있지 않다.

클린 빌드, 패키지 단독 실행, 배포 환경에서 import 순서 또는 런타임 의존성 문제가 발생할 수 있다.

개선 방향:

- 단기: `quvi_inspect`의 `package.xml`에 `quvi_robot_control` 의존성 추가
- 권장: 공용 유틸리티를 `quvi_common` 같은 별도 패키지로 분리

### 6. 레일 위치 파라미터가 실장비 스케일과 맞지 않아 보임
...
### 7. micro-ROS 연결 상태와 모터 상태가 ROS graph에 충분히 노출되지 않음
...
4. `quvi_common` 패키지 도입 또는 의존성 선언 보강
5. 레일 zone 위치 캘리브레이션 파라미터화
6. `/motor/status` 상태 메시지 추가
7. pytest 수집 범위와 테스트 규약 정리

## 결론

현재 QUVI는 기능별 패키지 분리, FSM 기반 오케스트레이션, HMI 통합, ESP32 펌웨어 설계가 갖춰진 시연형 시스템이다. 다만 실제 로봇 셀로 운용하려면 "명령을 보냈다"와 "하드웨어가 완료했다"를 명확히 분리해야 한다. 특히 E-STOP, 턴테이블 완료 피드백, ACT readiness는 안전성과 성공률에 직접 영향을 주므로 우선적으로 보완해야 한다.
