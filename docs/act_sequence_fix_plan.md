...
### P4 — 타이밍/검사 정합 (T1~T4)  ※ 구현 완료

1. **T1·T2 (해소):** P0로 HMI 개루프 시퀀스(`_run_sequence`)가 제거되어 개루프 고정 대기와
   검사 트리거 이중화가 구조적으로 사라졌다. 이제 단일 경로(오케스트레이터)의 피드백 기반
   (done 토픽·`_wait_motion_done`) 타이밍만 남는다.
2. **T4 (수정):** `inspect_node` 가 `/motor/turntable_done` 수신 즉시 캡처하던 것을,
   **안정화 지연(param `capture_settle_sec`=0.4s) 후 캡처**로 변경. 기구 진동·카메라 노출이
   안정된 프레임을 잡는다. 콜백 blocking sleep 은 이미지 갱신을 멈춰 오래된 프레임을 잡으므로,
   **비차단 일회성 타이머**(`_settle_timer` reset/cancel)로 구현.
3. **T3 (정리):** 오케스트레이터의 `/motor/rail_done` 이중 구독을 상태로 분기하는
   **단일 콜백 `_startup_rail_done_cb`** 로 통합.
...
- [x] (P4/T4) inspect 캡처 안정화 지연 추가 (param `capture_settle_sec`=0.4s, 비차단 타이머)
- [x] (P4/T3) `/motor/rail_done` 이중 구독 → 단일 상태분기 콜백으로 정리
