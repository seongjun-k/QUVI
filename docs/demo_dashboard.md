...
- 데모 컨트롤러(`quvi_hmi/demo_controller.py`): HMI "▶ 시작"(`/hmi/command`="START") 수신 시
  PASS/FAIL bag을 교대로 `ros2 bag play` (재생 중 재클릭·"STOP"/"ESTOP"에서 정지). demo.launch.py에 등록됨.
- rerun rrd 재서빙: `data/demo_bags/act.rrd`가 있으면 demo.launch.py가 `rerun --serve-web`을
  자동 실행(없으면 생략, 평시 무영향).
- `robot_control_node.py`에 `rerun_save_path` 파라미터 추가 — 실기 녹화 시 지정하면
  rr.save()로 rrd 파일 저장(rerun 0.22는 싱크가 단일이라 웹 뷰어와 동시 사용 불가, 저장 모드에서는 웹 뷰어 비활성).

## 남은 작업 (데모 완성까지)

1. 실기 테이크 2개 녹화: **양품 1사이클 + 불량품 1사이클** 각각 `ros2 bag record`
   (카메라·`/hmi/status` 등 대시보드 구독 토픽)을 `data/demo_bags/pass`, `data/demo_bags/fail`로 저장.
   같은 런에서 `rerun_save_path:=/workspace/data/demo_bags/act.rrd`를 robot_control_node에
   지정해 ACT 추론 rrd도 동시에 기록.
2. 폰 **세로** 촬영본(양품+불량 두 사이클 이어붙인 한 편)으로 `robot_overview.mp4` 교체
