...
The entire process is autonomously controlled by a finite state machine (FSM) orchestrator. Grasping is performed via LeRobot ACT imitation learning, and inspection combines surface-feature rule-based judgment with PatchCore anomaly detection (currently in shadow mode).
...
| **AI & Algorithms** | LeRobot ACT + OpenCV + PatchCore | Imitation-learning grasp control, surface-feature rule-based judgment, ML anomaly detection (shadow mode) |

### Inspection Method

The inspection camera captures images from 4 turntable angles (0°/90°/180°/270°) and analyzes them along two tracks.

1. **Surface-feature rule-based judgment (primary decision-maker)** — determines PASS/FAIL using the worst case across all angles
...
2. **PatchCore anomaly detection (shadow mode)** — computes an anomaly score using a WideResNet50-backbone, per-angle memory bank, and logs it only. It does not affect the final judgment; instead, it accumulates agreement rate against the rule-based judgment to validate readiness for cutover (`scripts/shadow_report.py`).

Reference images are generated by capturing known-good parts using the HMI's reference-image capture mode.
...
| **`quvi_inspect`** | `inspect_node` | 4-angle surface-feature pass/fail judgment + PatchCore anomaly detection (shadow), inspection log storage, reference-image / ML dataset capture modes |
...
## PatchCore Anomaly Detection Training Pipeline (Shadow Mode)
...
# 2. After shadow operation, report rule-vs-ML agreement rate (cutover gate)
...
- **Seoul Robotics High School Capstone Team "Pig Skin"**
